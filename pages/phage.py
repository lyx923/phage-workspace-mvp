...
from src.scientific.validator_service import (
    query_phages_for_host, batch_validate_hosts, query_l3_evidence,
    query_hosts_for_phage, validate_without_sequencing,
)
from src.scientific.evidence_package_service import (
    build_evidence_package_from_db, rule_based_evidence_package, verify_llm_effectiveness
)
from src.scientific.retriever_service import (
    find_matching_phages, find_similar_cases, analyze_and_persist_reuse, confirm_knowledge_reuse
)
from src.scientific.evidence_upgrade_service import (
    curate_case_by_id, review_evidence_upgrade_proposal,
    review_scientific_evidence_package, review_assay_qc
)

from src.shared.audit_service import write_audit_event

# 获取项目根目录 (注意：pages目录下需要退回到上一级)
...
def phage_typing_mode(driver):
    ...
    with tab2:
        ...
        if "batch_result" in st.session_state:
            st.dataframe(st.session_state.batch_result, width="stretch")
    ...
    with tab6:
        ...
        if st.button(f"查找 {', '.join(source_levels)} → {target_level_selector} 可升级记录"):
            ...
            if records:
                ...
                st.dataframe(df[["case_id", "phage_name", "evidence_level"]], width="stretch")
            else:
